...
MonckooseCollection.prototype.findAndModify = function (selector, sort, update, options, callback) {
  ...
  that.find(selector, options, function (err, result) {
    ...
    result.toArray(function (err, items) {
      var doneCounter, i, key, item, setArray, model;
      setArray = update['$set'];
      if (err) {
        callback && callback(err);
        return;
      ...
      for (i = 0; i < items.length; i++) {
        ...
        model = { _id: item._id };
        for (key in setArray) {
          model[key] = setArray[key];
        }
        that.update(model, function () {
          doneCounter++;
...
MonckooseCollection.prototype.update = function (item, callback) {
  var that, i, found, key;
  that = this;
  found = false;
